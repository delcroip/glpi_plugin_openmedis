 ...
 * ---------------------------------------------------------------------
 * Changes from Item_Device
 * Computer --> MedicalDevice 
 * Item_Device --> Item_MedicalAccessory
 * itemdevices --> itemmedicalaccessories
 */
...
class PluginOpenmedisDeviceMedicalAccessory extends CommonDBRelation {
   ...
   protected function MedicalDeviceFriendlyName() {
   ...
   **/
   ...
   function rawSearchOptions() {
      ...
      $tab[] = [
         ...
         'itemtype_list'      => 'itemmedicalaccessories_types',
         'nosort'             => true
      ];
    ...
    *
    * Should be overloaded by item_medicalaccessory*
    *
   ...
   **/
   ...
   /**
    * Get the items on which this item_medicalaccessory can be attached. For instance, a medicaldevice can have
    ...
    * "itemmedicalaccessoriesoundcard_types").
    *
    * Alternatively, it could be overloaded from subclasses
    *
    * @since 0.85
    *
    * @return array of the itemtype that can have this item_medicalaccessory
   **/
   static function itemAffinity() {
      ...
      return $CFG_GLPI["itemmedicalaccessories_itemaffinity"];
   }
    ...
    *
    * @return array of the types of item_medicalaccessory* available
   **/
   static function getDeviceTypes() {
      global $CFG_GLPI;

      // If the size of $CFG_GLPI['item_medicalaccessory_types'] and $CFG_GLPI['device_types'] then,
      // there is new device_types and we must update item_medicalaccessory_types !
      if (!isset($CFG_GLPI['item_medicalaccessory_types'])
          || (count($CFG_GLPI['item_medicalaccessory_types']) != count($CFG_GLPI['medical_accessory_types']))) {

         $CFG_GLPI['item_medicalaccessory_types'] = [];

         foreach (CommonDevice::getDeviceTypes() as $deviceType) {
            $CFG_GLPI['item_medicalaccessory_types'][] = $deviceType::getitem_medicalaccessoryType();
         }
      }

      return $CFG_GLPI['item_medicalaccessory_types'];
   }


   /**
    * Get the item_medicalaccessory* a given item type can have
    *
    * @param string $itemtype the type of the item that we want to know its devices
    *
    * @since 0.85
    *
    * @return array of item_medicalaccessory*
   **/
   static function getItemAffinities($itemtype) {
      global $GLPI_CACHE;

      if (!$GLPI_CACHE->has('item_medicalaccessory_affinities')) {
         $GLPI_CACHE->set('item_medicalaccessory_affinities', ['' => static::getDeviceTypes()]);
      }
      $items_affinities = $GLPI_CACHE->get('item_medicalaccessory_affinities');

      if (!isset($items_affinities[$itemtype])) {
         $afffinities = [];
         foreach ($items_affinities[''] as $item_id => $item_medicalaccessory) {
            if (in_array($itemtype, $item_medicalaccessory::itemAffinity()) || in_array('*', $item_medicalaccessory::itemAffinity())) {
               $afffinities[$item_id] = $item_medicalaccessory;
            }
         }
         $items_affinities[$itemtype] = $afffinities;
         $GLPI_CACHE->set('item_medicalaccessory_affinities', $items_affinities);
      }
   ...
   /**
    * Get all kind of items that can be used by item_medicalaccessory*
    *
    * @since 0.85
    *
   ...
   **/
   static function getConcernedItems() {
      ...
      $itemtypes = $CFG_GLPI['itemmedicalaccessories_types'];
   ...
   /**
    * Get associated device to the current item_medicalaccessory
    *
    * @since 0.85
    *
   ...
   **/
      ...
      if ($item->canView()) {
         ...
         if ($item instanceof CommonDevice) {
            if ($_SESSION['glpishow_count_on_tabs']) {
               ...
               $linkClass       = $deviceClass::getitem_medicalaccessoryType();
         ...
         if ($is_device) {
            ...
            Dropdown::showSelectItemFromItemtypes(['itemtype_name'       => 'devicetype',
                                                        ...
                                                        'showItemSpecificity' => $CFG_GLPI['root_doc']
                                                                 .'/ajax/selectUnaffectedOrNewitem_medicalaccessory.php']);
         }
         echo "</td><td>";
    ...
    * Two kind of item :
    *              - Device* feed by a link to the attached item (MedicalDevice, Printer ...)
    *              - MedicalDevice, Printer ...: feed by the "global" properties of the CommonDevice
    * Then feed with the specificities of the item_medicalaccessory elements
    * In cas of $item is an instance, then $options contains the type of the item (MedicalDevice,
    * Printer ...).
    *
    * @param $item
    * @param $table
   ...
   **/
   ...
   /**
    * Add one or several device(s) from front/PluginOpenmedisItem_MedicalAccessory.form.php.
    *
   ...
   **/
   static function addDevicesFromPOST($input) {
      ...
      if (isset($input['devicetype'])) {
         ...
         $linktype   = $devicetype::getitem_medicalaccessoryType();
         ...
         if ($item instanceof CommonDevice) {
            if ($link = getItemForItemtype($item->getitem_medicalaccessoryType())) {
   ...
   **/
   static function updateAll($input) {
      ...
      $is_device = ($item instanceof CommonDevice);
      if ($is_device) {
         $link_type = $itemtype::getitem_medicalaccessoryType();
      }
      ...
      foreach ($input as $key => $val) {
         ...
         if (!$is_device) {
            ...
            $device_type = $data[1];
            if (in_array($device_type::getitem_medicalaccessoryType(), self::getItemAffinities($itemtype))) {
               $link_type = $device_type::getitem_medicalaccessoryType();
            }
         }
         if (!empty($data[2])) {
            $links_id = $data[2];
    ...
    * @since 0.85
    *
    * @param $PluginOpenmedisItem_MedicalAccessory_id
    * @param $items_id
    * @param $itemtype
    *
    * @return boolean
   **/
   static function affectitem_medicalaccessory($PluginOpenmedisItem_MedicalAccessory_id, $items_id, $itemtype) {

      $link = new static();
      return $link->update(['id'       => $PluginOpenmedisItem_MedicalAccessory_id,
                                 'items_id' => $items_id,
                                 'itemtype' => $itemtype]);
   }
   ...
   **/
   ...
   function prepareInputForAdd($input) {
      ...
      $medicaldevice = static::getItemFromArray(static::$itemtype_1, static::$items_id_1, $input);

      if ($medicaldevice instanceOf CommonDBTM) {
         if (isset($CFG_GLPI['is_location_autoupdate']) && $CFG_GLPI["is_location_autoupdate"]
            && (!isset($input['locations_id']) ||
            $medicaldevice->fields['locations_id'] != $input['locations_id'])
         ) {
            $input['locations_id'] = $medicaldevice->fields['locations_id'];
         }

         if ((isset($CFG_GLPI['state_autoupdate_mode']) && $CFG_GLPI["state_autoupdate_mode"] < 0)
            && (!isset($input['states_id']) ||
            $medicaldevice->fields['states_id'] != $input['states_id'])
         ) {

            $input['states_id'] = $medicaldevice->fields['states_id'];
         }
   ...
   static function getSearchURL($full = true) {
      ...
      $link = "$dir/front/item_medicalaccessory.php?itemtype=$itemtype";
